...
import cn.dd.ori1st.SynLastModifyTime.iteratorFunctionLT
...
/**
  ...
  * Version: V1.0
  * Author: ChengTao
  * Description: 增量数据更新集成程序(源数据转换（仅资金）+标签标定+ES数据同步)
  */
object SynIncDataV1 {
  //系统配置文件（存放于项目resources目录下）
  private val CONFIG_FILE: String = "syn_inc_data_v1.conf"
  //设置Phoenix驱动
  ...
  def main(args: Array[String]): Unit = {
    //增量范围开始时间
    val dataRangeBegin:String = args(1)+" 00:00:00"//"2020-07-08 00:00:00"
    //增量范围结束时间
    val dataRangeEnd:String = args(2)+" 23:59:59"//"2020-07-08 23:59:59"
    //索引后缀
    val esIncEndness:String = args(0)//"2020-05-30"
    //读取并加载项目配置文件
    ...
    val conf = new SparkConf().setAppName(SynIncDataV1.getClass.getName)
    ...
    taskList.map(x=>{
      ...
      val taskDetail:String = confData.getProperty(x.toString).replace("#START#",dataRangeBegin).replace("#END#",dataRangeEnd)
      val taskInfo: (String, String, String, String, String, String) = DataFormat.getTaskInfo(taskName,taskDetail)
      dataOptionDeal(taskInfo,ss,zkUrl,esIncEndness)
    })
    ss.stop()
  ...
  /**
    * 数据处理函数
    * */
  def dataOptionDeal(taskInfo:Tuple6[String, String, String, String, String, String],ss:SparkSession,zkUrl:String,esIncEndness:String):Unit={
    ...
    }else if(taskInfo._5.equals("SGTO")){
      //打标签处理（暂限于 HBASE -> HBASE）
      tagOptionDeal(taskInfo._2,taskInfo._6,ss,zkUrl)

    }else if(taskInfo._5.equals("ESTO") && taskInfo._1.toInt>44){
  ...
  def dataSyn2ES(optTable:String,optSQL: String,ss:SparkSession,zkUrl:String,esIncEndness:String):Unit={
    ...
    println("to update index[%s], alias[%s], id field[%s], handle sql:[%s]\n".format(trueIndex, optTable, idName, optParam._2))//用于代码跟踪，上线时请删除
    val dfOri = ss.sql(optParam._2).cache()//将查询结果转为DataFrame
    val c = dfOri.count()
  ...
  def dataSynUPES(optTable:String,optSQL: String,ss:SparkSession,zkUrl:String,esIncEndness:String):Unit={
    ...
    if(tagHead.equals("101_")){
      ...
      EsSpark.saveToEs(rdd, trueIndex, Map("es.mapping.id" -> "ent_id", "es.write.operation" -> "update"))
    }else if(tagHead.equals("lmt_")){
      val rdd = dfOri.rdd.mapPartitions(iteratorFunctionLT(_))
      println("to update index for nested,[%s]/[%s][%s]".format(trueIndex, "last_modify_time",tagHead))
      EsSpark.saveToEs(rdd, trueIndex, Map("es.mapping.id" -> "ent_id", "es.write.operation" -> "update"))
    }
    dfOri.unpersist()
  }
